                    ...
                    <CardContent sx={{ padding: 0 }}>
                        <Box sx={{ padding: 2 }}>
                            <TrackerActiveCardInfoRow>
                                ...
                                <Divider orientation="vertical" flexItem />
                                <NumberSetting
                                    settingTitle={t('manga.label.status')}
                                    dialogTitle={t('manga.label.status')}
                                    settingValue={`${trackRecord.score}`}
                                    value={trackRecord.score ?? 0}
                                    minValue={Number(tracker.scores[0])}
                                    maxValue={Number(tracker.scores.slice(-1)[0])}
                                    stepSize={Number(tracker.scores[1]) - Number(tracker.scores[0])}
                                    valueUnit=""
                                    handleUpdate={(score) => updateTrackerBind({ scoreString: `${score}` })}
                                />
                            </TrackerActiveCardInfoRow>
                            <Divider />
                            <TrackerActiveCardInfoRow>
