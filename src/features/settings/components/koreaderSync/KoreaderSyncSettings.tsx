...
export const KoreaderSyncSettings = ({
    ...
    const handleLogin = async (username: string, password: string, serverUrl: string) => {
        const { data } = await requestManager.koSyncLogin(username, password, serverUrl).response;
    ...
    const handleLoginLogout = async (
        ...
        initialServerAddress: string = koreaderSyncServerUrl,
    ) => {
        const controlled = CredentialsLogin.showControlled(
            {
                title: t(
                    isLoggedIn ? 'tracking.settings.dialog.title.log_out' : 'tracking.settings.dialog.title.log_in',
                    {
                ...
                serverUrl: initialServerAddress,
                loginLogout: async (username, password, serverUrl) => {
                    ...
                    if (isLoggedIn) {
                        try {
                            ...
                            makeToast(
                                t('settings.server.koreader.sync.message.disconnect_error'),
                                'error',
                                getErrorMessage(e),
                            );
                        }
                    ...
                    try {
                        ...
                        await handleLogin(username, password, serverUrl);

                        controlled.submit();
                    } catch (e) {
                        makeToast(
                            t('settings.server.koreader.sync.message.connect_error'),
                            'error',
                            getErrorMessage(e),
                        );
                        ...
                        if (retry === RETRY_KEY) {
                            handleLoginLogout(username, password, serverUrl);
                        }
                    }
                },
            },
    ...
    return (
        ...
        >
            <ListItemButton onClick={() => handleLoginLogout()}>
                <ListItemText
                    primary={t('settings.server.koreader.sync.status')}
                    secondary={
                        isLoggedIn
                            ? t('settings.server.koreader.sync.logged_in_as', {
                                  username: currentUsername,
                                  serverAddress: koreaderSyncServerUrl,
                              })
                            : t('global.label.logged_out')
                    }
                />
                {isLoggedIn && (
