...
import { getChapterIdsForDownloadAhead, getReaderChapterFromCache } from '@/modules/reader/utils/Reader.utils.ts';
        ...
        return useCallback(
            (pageIndex, debounceChapterUpdate = true, endReached = false) => {
                ...
                if (chapterIdsForDownloadAhead.length) {
                    Chapters.download(chapterIdsForDownloadAhead).catch(
