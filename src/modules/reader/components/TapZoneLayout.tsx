...
export const TapZoneLayout = () => {
    ...
    const canvas = ReaderTapZoneService.getOrCreateCanvas(TapZoneLayouts.L_SHAPE, width, height, theme.typography.h3, {
        vertical: false,
        horizontal: false,
    });

    useLayoutEffect(() => {
        const test = document.getElementById(CANVAS_ID);
        canvas.id = CANVAS_ID;
        test?.replaceWith(canvas);
    }, [canvas]);
    ...
    return (
        <Box
            ...
            sx={{ position: 'fixed', top: 0, left: readerNavBarWidth, right: 0, bottom: 0 }}
        >
            <canvas id={CANVAS_ID} />
        </Box>
    );
