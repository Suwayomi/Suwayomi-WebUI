...
const DownloadButton = ({
    ...
    if (Chapters.isDownloaded(currentChapter)) {
        return (
    ...
    return (
        <Tooltip title={t(actionToTranslationKey.download.action.single)}>
            <IconButton onClick={() => Chapters.performAction('download', [currentChapter.id], {})} color="inherit">
                <DownloadIcon />
            </IconButton>
        </Tooltip>
    );
...
export const ReaderNavBarDesktopActions = ({
    ...
    const { id, isBookmarked, realUrl } = currentChapter;
    ...
    return (
        <Stack sx={{ flexDirection: 'row', justifyContent: 'center', gap: 1 }}>
            ...
            <Tooltip title={t('chapter.action.label.open_on_source')}>
                <IconButton disabled={!realUrl} href={realUrl ?? ''} rel="noreferrer" target="_blank">
                    <OpenInNewIcon />
                </IconButton>
            </Tooltip>
        </Stack>
