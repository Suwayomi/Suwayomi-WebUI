...
export const ReaderOverlayHeaderMobile = ({
    manga: { title },
    chapter: { name, realUrl, isBookmarked },
    isVisible,
}: MobileHeaderProps) => {
    ...
    return (
        <Slide direction="down" in={isVisible} mountOnEnter unmountOnExit>
            ...
            >
                ...
                <Tooltip title={t(actionToTranslationKey[isBookmarked ? 'unbookmark' : 'bookmark'].action.single)}>
                    <IconButton color="inherit">{isBookmarked ? <BookmarkIcon /> : <BookmarkBorderIcon />}</IconButton>
                </Tooltip>
